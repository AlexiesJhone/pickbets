...
class gradingpick3 implements ShouldQueue
{
    ...
    public function handle()
    {
      ...
      DB::transaction(function () use($fightnumber,$results,$event_id,$confirm1,$confirm2,$data,$name,$a,$Starting){
        ...
        $checkfightnumber = $fightnumber-2;
        $CHECKPOTMONEY = Potmoney::whereIn('event_id',$a)->where('startingfight',$checkfightnumber)->where('pick',3)->first();
        if (!$CHECKPOTMONEY) {
          return 'wala';
        }
        $checkevent = Event::where('id',$CHECKPOTMONEY->event_id)->where('pick',3)->first();
        if (isset($CHECKPOTMONEY)) {
          $lastfight = $CHECKPOTMONEY->startingfight+2;
        if ($CHECKPOTMONEY) {
          ...
          if ($checkforcancel==1) {
            ...
            $maxwin=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',3)->first();
            ...
            $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',2)->whereIn('event_id',$a)->where('turn',3)->update([
                'lose' => DB::raw('turn-wins'),
                'winner' => 1
              ]);

              $winners = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->where('turn',3)->get();
            // }
            ...
            if ($dividendo<$minimumpay) {
              expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->where('turn',3)->update([
                'lose' => DB::raw('turn-wins'),
                'winner' => 4
              ]);
            }else {
              ...
              if ($checkifmeron) {
                expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('winner',0)->whereIn('event_id',$a)->where('turn',3)->update([
                  'lose' => DB::raw('turn-wins'),
                  'winner' => 3
                ]);
              }else {
            ...
            Potmoney::where('startingfight',$CHECKPOTMONEY->startingfight)->where('event_id',$CHECKPOTMONEY->event_id)->where('pick',3)->update(['claim'=>1]);
            ...
            $checknextbets = expertbet::where('startingfight',$nextstart)->whereIn('event_id',$a)->where('turn',3)->count();
            if (!$checknextbets) {
              Potmoney::whereIn('event_id',$a)->where('startingfight',$nextstart)->where('pick',3)->update(['claim'=>1]);
              Event::where('startingfight',$nextstart)->WhereIn('id',$a)->where('pick',3)->update(['control'=>'Finished']);
            }
          ...
          elseif ($checkforcancel==2) {
            ...
            // $maxwin2=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',2)->select('wins')->groupBy('wins')->first();
              // foreach ($maxwin2 as $key) {
            $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',1)->whereIn('event_id',$a)->where('turn',3)->update([
                'lose' => DB::raw('turn-wins'),
                'winner' => 1
              ]);
          ...
          elseif ($checkforcancel==3) {
            $check=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('winner',0)->whereIn('event_id',$a)->where('turn',3)->update([
              'lose' => DB::raw('turn-wins'),
              'winner' => 4
            ]);
            Event::where('startingfight',$CHECKPOTMONEY->startingfight)->where('id',$CHECKPOTMONEY->event_id)->update(['control'=>'Finished']);
            ...
            // $maxwin2=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',2)->select('wins')->groupBy('wins')->first();
              // foreach ($maxwin2 as $key) {
              // declaration of all Winners
              $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',3)->whereIn('event_id',$a)->where('turn',3)->update([
               'lose' => DB::raw('turn-wins'),
               'winner' => 1
             ]);
          ...
          Event::where('startingfight',$fightnumber)->whereIn('id',$a)->where('pick',3)->update(['control'=>'Finished']);
          $nextstart = $CHECKPOTMONEY->startingfight + 1;
          $checknextbets = expertbet::where('startingfight',$nextstart)->whereIn('event_id',$a)->where('turn',3)->count();
          if (!$checknextbets) {
            Potmoney::whereIn('event_id',$a)->where('startingfight',$nextstart)->where('pick',3)->update(['claim'=>1]);
