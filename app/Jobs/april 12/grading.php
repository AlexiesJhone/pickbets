...
class grading implements ShouldQueue
{
    ...
    public function handle()
    {
      ...
      DB::transaction(function () use($fightnumber,$results,$event_id,$confirm1,$confirm2,$data,$name,$a){
        ...
        $data2=$data1+1;
        $data3=$data2+$control->pick+1;

        $checkforcancel = Results::whereIn('event_id',$a)->where('result','Cancelled')->whereBetween('fightnumber', [$data2, $data3])->count();
        if ($checkforcancel==1) {
          $data2=$data1;
          $checker = expertbet::where('startingfight',$data2)->whereIn('event_id',$a)->where('winner',0)->orderBy('wins','DESC')->first();
        }elseif ($checkforcancel==2 || $checkforcancel>2) {
          $data2=$data1-1;
          $checker = expertbet::where('startingfight',$data2)->whereIn('event_id',$a)->where('winner',0)->orderBy('wins','DESC')->first();
        }
        else {
