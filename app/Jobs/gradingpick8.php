...
class gradingpick8 implements ShouldQueue
{
    ...
    public function handle()
    {
      ...
      DB::transaction(function () use($fightnumber,$results,$event_id,$confirm1,$confirm2,$data,$name,$a,$Starting){
        ...
        $checkfightnumber = $fightnumber-7;
        $CHECKPOTMONEY = Potmoney::whereIn('event_id',$a)->where('startingfight',$checkfightnumber)->where('pick',8)->first();
        if (!$CHECKPOTMONEY) {
          return 'wala';
        }
        $checkevent = Event::where('id',$CHECKPOTMONEY->event_id)->where('pick',8)->first();
        if (isset($CHECKPOTMONEY)) {
          $lastfight = $CHECKPOTMONEY->startingfight+7;
        if ($CHECKPOTMONEY) {
          ...
          if ($checkforcancel==1) {
            ...
            $maxwin=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',8)->first();
            ...
            $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',7)->whereIn('event_id',$a)->where('turn',8)->update([
                'lose' => DB::raw('turn-wins'),
                'winner' => 1
              ]);

              $winners = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->where('turn',8)->get();
            // }
            ...
            if ($dividendo<$minimumpay) {
              expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->where('turn',8)->update([
                'lose' => DB::raw('turn-wins'),
                'winner' => 4
              ]);
            }else {
              ...
              if ($checkifmeron) {
                expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('winner',0)->whereIn('event_id',$a)->where('turn',8)->update([
                  'lose' => DB::raw('turn-wins'),
                  'winner' => 3
                ]);
              }else {
            ...
            Potmoney::where('startingfight',$CHECKPOTMONEY->startingfight)->where('event_id',$CHECKPOTMONEY->event_id)->where('pick',8)->update(['claim'=>1]);
          ...
          elseif ($checkforcancel==2) {
            ...
            // $maxwin2=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',2)->select('wins')->groupBy('wins')->first();
              // foreach ($maxwin2 as $key) {
            $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',6)->whereIn('event_id',$a)->where('turn',8)->update([
                'lose' => DB::raw('turn-wins'),
                'winner' => 1
              ]);
          ...
          elseif ($checkforcancel==3) {
            ...
            // $maxwin2=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',2)->select('wins')->groupBy('wins')->first();
              // foreach ($maxwin2 as $key) {
            $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',5)->whereIn('event_id',$a)->where('turn',8)->update([
                'lose' => DB::raw('turn-wins'),
                'winner' => 1
              ]);
          ...
          elseif ($checkforcancel==4) {
            ...
            // $maxwin2=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',2)->select('wins')->groupBy('wins')->first();
              // foreach ($maxwin2 as $key) {
            $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',4)->whereIn('event_id',$a)->where('turn',8)->update([
                'lose' => DB::raw('turn-wins'),
                'winner' => 1
              ]);
          ...
          elseif ($checkforcancel==5) {
            ...
            // $maxwin2=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',2)->select('wins')->groupBy('wins')->first();
              // foreach ($maxwin2 as $key) {
            $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',3)->whereIn('event_id',$a)->where('turn',8)->update([
                'lose' => DB::raw('turn-wins'),
                'winner' => 1
              ]);
          ...
          elseif ($checkforcancel==6) {
            ...
            // $maxwin2=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',2)->select('wins')->groupBy('wins')->first();
              // foreach ($maxwin2 as $key) {
            $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',2)->whereIn('event_id',$a)->where('turn',8)->update([
                'lose' => DB::raw('turn-wins'),
                'winner' => 1
              ]);
          ...
          elseif ($checkforcancel==7) {
            ...
            // $maxwin2=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',2)->select('wins')->groupBy('wins')->first();
              // foreach ($maxwin2 as $key) {
            $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',1)->whereIn('event_id',$a)->where('turn',8)->update([
                'lose' => DB::raw('turn-wins'),
                'winner' => 1
              ]);
          ...
          elseif ($checkforcancel==8) {
            $check=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('winner',0)->whereIn('event_id',$a)->where('turn',8)->update([
              'lose' => DB::raw('turn-wins'),
              'winner' => 4
            ]);
            Event::where('startingfight',$CHECKPOTMONEY->startingfight)->where('id',$CHECKPOTMONEY->event_id)->update(['control'=>'Finished']);
            ...
            // $maxwin2=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',2)->select('wins')->groupBy('wins')->first();
              // foreach ($maxwin2 as $key) {
              // declaration of all Winners
              $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',8)->whereIn('event_id',$a)->where('turn',8)->update([
               'lose' => DB::raw('turn-wins'),
               'winner' => 1
             ]);
          ...
          Event::where('startingfight',$fightnumber)->whereIn('id',$a)->where('pick',8)->update(['control'=>'Finished']);
          event(new leaderboards($data));
