...
class grading implements ShouldQueue
{
    ...
    public function handle()
    {
      ...
      DB::transaction(function () use($fightnumber,$results,$event_id,$confirm1,$confirm2,$data,$name,$a,$Starting){
        ...
        if ($checkforcancel==1) {
          $data2=$data1;
          $checker = expertbet::where('startingfight',$data2)->whereIn('event_id',$a)->where('winner',0)->orderBy('wins','DESC')->first();
        ...
        if ($checker===null) {
          ...
          $lowest = expertbet::where('startingfight',$data2)->whereIn('event_id',$a)->where('winner',3)->select('wins')->orderBy('wins')->first();
          expertbet::where('startingfight',$data2)->where('winner',3)->whereIn('wins', [0, 1, 2, 3])->whereIn('event_id',$a)->update([
            'lose' => DB::raw('turn-wins'),
            'winner' => 1
          ]);
