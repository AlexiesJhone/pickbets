        ...
        if ($getevent->currentfight >= $lastnumber) {
          ...
          if ($checker===null) {
            ...
            // $maxwin=expertbet::where('startingfight',$getbet->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->first();
              ...
              if ($checkforcancel==1) {
              $checkifmeron = expertbet::where('startingfight',$getbet->startingfight)->where('wins',1)->where('turn',2)->whereIn('event_id',$a)->update([
              ...
              $checkifmeron = expertbet::where('startingfight',$getbet->startingfight)->where('wins',2)->where('turn',2)->whereIn('event_id',$a)->update([
                  'lose' => DB::raw('turn-wins'),
                  'winner' => 1
                ]);
                $winners = expertbet::where('startingfight',$getbet->startingfight)->whereIn('event_id',$a)->where('turn',2)->get();
