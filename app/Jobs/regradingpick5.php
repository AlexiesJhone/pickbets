...
class regradingpick5 implements ShouldQueue
{
    ...
    public function handle()
    {
      ...
      $lastnumber = $startingfight->startingfight + 4;
      ...
      ->where('a.fightnumber', '=', $fightnumber)->whereIn('a.event_id', $a)->where('a.selection', '=', $lumangresult)->where('c.turn',5)
      ->update(['c.wins' =>DB::raw('wins - 1')]);

        DB::table('selection as a')
      ->where('a.fightnumber', '=', $fightnumber)->whereIn('a.event_id', $a)->where('a.selection', '=', $bagongresult)->where('c.turn',5)
      ->join('expertbet as c', 'a.expertbet_id', '=', 'c.id')
      ->update(['c.wins' =>DB::raw('wins + 1')]);

      $getevent=Event::whereIn('id',$a)->where('pick',5)->first();
      $minimumpay = $getevent->payout+1;
      $control = control::first();
        $getbet = expertbet::where('id',$startingfight->id)->whereIn('event_id',$a)->where('turn',5)->first();
		      $checkforcancel = Results::whereIn('event_id',$a)->where('result','Cancelled')->whereBetween('fightnumber', [$startingfight->startingfight, $lastnumber])->count();
        $computed1 = true;
        if ($getevent->currentfight >= $lastnumber) {
          ...
          // $data2=$data1+1;
          $checker = expertbet::where('startingfight',$getbet->startingfight)->whereIn('event_id',$a)->where('turn',5)->orderBy('wins','DESC')->first();
          if ($checker===null) {
            return $getbet->startingfight;
          }else {
            expertbet::where('startingfight',$getbet->startingfight)->whereIn('event_id',$a)->where('turn',5)->update(['winner'=>0]);
            // $maxwin=expertbet::where('startingfight',$getbet->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->first();
              ...
              if ($checkforcancel==1) {
              $checkifmeron = expertbet::where('startingfight',$getbet->startingfight)->where('wins',4)->where('turn',5)->whereIn('event_id',$a)->update([
                  'lose' => DB::raw('turn-wins'),
                  'winner' => 1
                ]);
                $winners = expertbet::where('startingfight',$getbet->startingfight)->whereIn('event_id',$a)->where('turn',5)->get();
              // }
              ...
              if ($dividendo<130) {
                expertbet::where('startingfight',$getbet->startingfight)->whereIn('event_id',$a)->where('turn',5)->update([
                  'lose' => DB::raw('turn-wins'),
                  'winner' => 4
                ]);
              }else {
                ...
                if ($checkifmeron) {
                  expertbet::where('startingfight',$getbet->startingfight)->where('winner',0)->whereIn('event_id',$a)->where('turn',5)->update([
                    'lose' => DB::raw('turn-wins'),
                    'winner' => 3
                  ]);
                }else {
            ...
            elseif ($checkforcancel==2) {
              $checkifmeron = expertbet::where('startingfight',$getbet->startingfight)->where('wins',3)->where('turn',5)->whereIn('event_id',$a)->update([
                  'lose' => DB::raw('turn-wins'),
                  'winner' => 1
                ]);
            ...
            elseif ($checkforcancel==3) {
              $checkifmeron = expertbet::where('startingfight',$getbet->startingfight)->where('wins',2)->where('turn',5)->whereIn('event_id',$a)->update([
                  'lose' => DB::raw('turn-wins'),
                  'winner' => 1
                ]);
            ...
            elseif ($checkforcancel==4) {
              $checkifmeron = expertbet::where('startingfight',$getbet->startingfight)->where('wins',1)->where('turn',5)->whereIn('event_id',$a)->update([
                  'lose' => DB::raw('turn-wins'),
                  'winner' => 1
                ]);
            ...
            elseif ($checkforcancel==5) {
                expertbet::where('startingfight',$getbet->startingfight)->where('turn',5)->whereIn('event_id',$a)->update([
                  'lose' => DB::raw('turn-wins'),
                  'winner' => 4
                ]);
                // event(new resultevent('alex'.$startingfight->id.' '.$getbet,$isangcancel,$dalawangcancel,$tatlongcancel,'cancelcount ay '.$checkforcancel,'last number ay '.$lastnumber,'fught number ay'.$fightnumber,''));
              }else{
              $checkifmeron = expertbet::where('startingfight',$getbet->startingfight)->where('wins',5)->where('turn',5)->whereIn('event_id',$a)->update([
                  'lose' => DB::raw('turn-wins'),
                  'winner' => 1
                ]);
                $winners = expertbet::where('startingfight',$getbet->startingfight)->whereIn('event_id',$a)->where('turn',5)->get();
            ...
            Event::whereIn('id',$a)->where('startingfight',$startingfight->startingfight)->where('pick',5)->update(['control' =>'Finished']);
            // expertbet::where('startingfight',$getbet->startingfight)->where('winner',0)->whereIn('event_id',$a)->where('turn',5)->update(['winner'=>3, 'lose'=> DB::raw('turn-wins')]);
          }
