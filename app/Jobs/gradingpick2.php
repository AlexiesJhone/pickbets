...
class gradingpick2 implements ShouldQueue
{
    ...
    public function handle()
    {
      ...
      DB::transaction(function () use($fightnumber,$results,$event_id,$confirm1,$confirm2,$data,$name,$a,$Starting){
        ...
        $checkevent = Event::where('id',$CHECKPOTMONEY->event_id)->where('pick',2)->first();
        ...
        if ($CHECKPOTMONEY) {
          ...
          if ($checkforcancel==1) {
            ...
            $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',1)->whereIn('event_id',$a)->where('turn',2)->update([
            ...
            $checknextbets = expertbet::where('startingfight',$nextstart)->whereIn('event_id',$a)->where('turn',2)->count();
            if (!$checknextbets) {
              Potmoney::whereIn('event_id',$a)->where('startingfight',$nextstart)->where('pick',2)->update(['claim'=>1]);
              Event::where('startingfight',$nextstart)->WhereIn('id',$a)->update(['control'=>'Finished']);
            }
          ...
          elseif ($checkforcancel==2) {
            ...
            // $maxwin2=expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',2)->select('wins')->groupBy('wins')->first();
              // foreach ($maxwin2 as $key) {
              // declaration of all Winners
              $checkifmeron = expertbet::where('startingfight',$CHECKPOTMONEY->startingfight)->where('wins',2)->whereIn('event_id',$a)->where('turn',2)->update([
               'lose' => DB::raw('turn-wins'),
               'winner' => 1
             ]);
