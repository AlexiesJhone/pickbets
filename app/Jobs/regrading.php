...
class regrading implements ShouldQueue
{
    ...
    //event(new resultevent($lastnumbernocancel,$isangcancel,$dalawangcancel,$tatlongcancel,'cancelcount ay '.$checkforcancel,'last number ay '.$lastnumber,'fught number ay'.$fightnumber,''));
      DB::transaction(function () use($cc1,$named,$fightnumber,$checkforcancel,$isangcancel,$a,$lumangresult,$bagongresult,$lastnumber,$startingfight,$lastnumbernocancel,$dalawangcancel,$tatlongcancel){

      // return $checkforcancel;
      if ($checkforcancel==1 && $fightnumber<=$isangcancel) {
        DB::table('selection as a')
      ->where('a.fightnumber', '=', $fightnumber)->whereIn('a.event_id', $a)->where('a.selection', '=', $lumangresult)->where('c.turn',20)
      ->join('expertbet as c', 'a.expertbet_id', '=', 'c.id')
      ->update(['c.wins' =>DB::raw('wins - 1')]);

        DB::table('selection as a')
      ->where('a.fightnumber', '=', $fightnumber)->whereIn('a.event_id', $a)->where('a.selection', '=', $bagongresult)->where('c.turn',20)
      ->join('expertbet as c', 'a.expertbet_id', '=', 'c.id')
      ->update(['c.wins' =>DB::raw('wins + 1')]);
      if ($bagongresult=="Cancelled" && $fightnumber!=$lastnumber) {
        DB::table('selection as a')
      ->where('a.fightnumber', '=', $fightnumber)->whereIn('a.event_id', $a)->where('c.turn',20)
      ->join('expertbet as c', 'a.expertbet_id', '=', 'c.id')
      ->update(['c.lose' =>null,'c.winner'=>0]);
      Event::where('pick',20)->whereIn('id',$a)->where('startingfight',$startingfight->startingfight)
    ...
    //event(new resultevent('Last','endevent','alex2','eventupdate','id','alex2','id','id'));
      ...
      $getevent=Event::whereIn('id',$a)->where('pick',20)->first();
      $control = control::first();
        $getbet = expertbet::where('startingfight',$startingfight->startingfight)->whereIn('event_id',$a)->where('turn',20)->first();
        event(new resultevent('Last','endevent','alex21','eventupdate','id','alex2','id','id'));
        ...
        if ($getevent->currentfight >= $computed) {
          ...
          $data2=$data1+1;
          $checker = expertbet::where('startingfight',$getbet->startingfight)->whereIn('event_id',$a)->where('turn',20)->orderBy('wins','DESC')->first();
          if ($checker===null) {
            return $getbet->startingfight;
          }else {
            expertbet::where('startingfight',$getbet->startingfight)->whereIn('event_id',$a)->where('turn',20)->update(['winner'=>0]);
            $maxwin=expertbet::where('startingfight',$getbet->startingfight)->whereIn('event_id',$a)->where('turn',20)->orderBy('wins','DESC')->first();
            $maxwin2=expertbet::where('startingfight',$getbet->startingfight)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',20)->select('wins')->groupBy('wins')->get()->take($control->numberofwinners);
            ...
            // }else {
              foreach ($maxwin2 as $key) {
              expertbet::where('startingfight',$getbet->startingfight)->where('wins',$key->wins)->where('turn',20)->whereIn('event_id',$a)->update([
                'lose' => DB::raw('turn-wins'),
                'winner' => 1
              ]);
            }
            ...
            //   'winner' => 1
            // ]);
            Event::whereIn('id',$a)->where('startingfight',$startingfight->startingfight)->where('pick',20)
            ->update(['control' =>'Finished']);
            Potmoney::where('startingfight',$maxwin->startingfight)->where('event_id',$maxwin->event_id)->where('pick',20)->update(['claim'=>1]);
            expertbet::where('startingfight',$getbet->startingfight)->where('winner',0)->where('turn',20)->whereIn('event_id',$a)->update(['winner'=>3, 'lose'=> DB::raw('turn-wins')]);
            $lowest = expertbet::where('startingfight',$getbet->startingfight)->where('turn',20)->whereIn('event_id',$a)->where('winner',3)->select('wins')->orderBy('wins')->first();
            expertbet::where('startingfight',$getbet->startingfight)->where('winner',3)->where('turn',20)->whereIn('wins', [0, 1, 2, 3])->whereIn('event_id',$a)->update([
              'lose' => DB::raw('turn-wins'),
              'winner' => 1
            ]);
          }
