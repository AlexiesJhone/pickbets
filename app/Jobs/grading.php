...
class grading implements ShouldQueue
{
    ...
    public function handle()
    {
      ...
      DB::transaction(function () use($fightnumber,$results,$event_id,$confirm1,$confirm2,$data,$name,$a,$Starting){
        ...
        ->where('a.fightnumber', '=', $fightnumber)->whereIn('a.event_id', $a)->where('a.selection', '=', $results)->where('winner',0)
        ...
        $checkforcancel = Results::whereIn('event_id',$a)->where('result','Cancelled')->whereBetween('fightnumber', [$Starting, $data3])->count();

        if ($checkforcancel==1) {
          $data2=$data1;
          $checker = expertbet::where('startingfight',$data2)->whereIn('event_id',$a)->where('winner',0)->where('turn',20)->orderBy('wins','DESC')->first();
          //event(new resultevent($results,$fightnumber,$name,'starting '.$data2,1,$confirm2,$results,$confirm1,$confirm1));
        }elseif ($checkforcancel==2 ) {
          $data2=$data1-1;
          $checker = expertbet::where('startingfight',$data2)->whereIn('event_id',$a)->where('winner',0)->where('turn',20)->orderBy('wins','DESC')->first();
        }elseif ($checkforcancel==3 || $checkforcancel>3) {
          $data2=$data1-2;
          $checker = expertbet::where('startingfight',$data2)->whereIn('event_id',$a)->where('winner',0)->where('turn',20)->orderBy('wins','DESC')->first();
        ...
        if ($checker===null) {
          ...
          $maxwin=expertbet::where('startingfight',$data2)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',20)->first();
          $maxwin2=expertbet::where('startingfight',$data2)->whereIn('event_id',$a)->orderBy('wins','DESC')->where('turn',20)->select('wins')->groupBy('wins')->get()->take($control->numberofwinners);
          ...
          // else {
            foreach ($maxwin2 as $key) {
            expertbet::where('startingfight',$data2)->where('wins',$key->wins)->whereIn('event_id',$a)->where('turn',20)->update([
              'lose' => DB::raw('turn-wins'),
              'winner' => 1
            ]);
          }
          ...
          Event::where('startingfight',$maxwin->startingfight)->where('id',$maxwin->event_id)->where('pick',20)->update(['control'=>'Finished']);

            // DECLARATION OF THE LOSERS


          expertbet::where('startingfight',$data2)->where('winner',0)->whereIn('event_id',$a)->where('turn',20)->update([
            'lose' => DB::raw('turn-wins'),
            'winner' => 3
          ]);

          $lowest = expertbet::where('startingfight',$data2)->whereIn('event_id',$a)->where('winner',3)->where('turn',20)->select('wins')->orderBy('wins')->first();
          expertbet::where('startingfight',$data2)->where('winner',3)->whereIn('wins', [0, 1, 2, 3])->where('turn',20)->whereIn('event_id',$a)->update([
            'lose' => DB::raw('turn-wins'),
            'winner' => 1
          ]);
