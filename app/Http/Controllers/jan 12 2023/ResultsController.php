...
class ResultsController extends Controller
{
  ...
  public function confirmgrade(Request $req)
  {
    ...
    $createlogs->user_id = auth()->user()->id;
    $createlogs->message = auth()->user()->username.' requested regrade '.$req['newresult'].' for fight number '.$req['fightnumberregrade'];
    $createlogs->save();
  ...
  public function regrade(Request $req)
  {
    ...
    if ($getoneprebetonly||$getoneprebetonly2) {
      if ($getoneprebetonly) {
  ...
  public function confirmed(Request $req)
  {
    ...
    // $data2->save();
      ...
      if ($getoneprebetonly) {
        ...
        $getonebetonly1 = expertbet::where('id',$getoneprebetonly->expertbet_id)->first();
        // return $getoneprebetonly->expertbet_id;
          ...
          $this->dispatch(new gradingpick2($result,$eventid,$fn,$c1,$c2,auth()->user()->name,$getonebetonly1->startingfight,$array));
      }else{
  ...
  public function insertresults(Request $req)
  {
    ...
    $createlogs->user_id = auth()->user()->id;
    $createlogs->message = auth()->user()->username.' Requested '.$req['result'].' for fight number '.$req['fightnumber'].' to '.$getrequesteddeclarator->username.'.';
    $createlogs->save();
