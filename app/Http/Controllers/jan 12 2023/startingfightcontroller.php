...
class startingfightcontroller extends Controller
{
  ...
  public function insertstartingfight(Request $req)
  {
    ...
    $getactiveevent = Event::where('id',$req['event_id'])->where('event_name',$getnameevent)->where('startingfight',$req['startingfight'])->where('status',1)->latest()->first();
    if ($getactiveevent) {
      return error;
    }else {
      $control = control::first();
      $getevent = Event::where('event_name',$getnameevent->event_name)->where('id',$req['event_id'])->where('status',1)->where('pick',$control->pick)->latest()->first();
      ...
      if ($req->startingfight > $computed && $lastfight <= $getevent->fights) {
        ...
        $newcheck = Event::where('event_name',$getnameevent->event_name)->where('id',$req['event_id'])->where('status',1)->where('pick',$control->pick)->latest()->first();
