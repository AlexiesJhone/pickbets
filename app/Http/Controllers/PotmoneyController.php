...
return Potmoney::whereIn('event_id',$array)->orderBy('startingfight','desc')->get();
  ...
  public function getliveodds(Request $req)
  {
    ...
    if ($total) {
      if ($req['pick']==2||$req['pick']==3||$req['pick']==4||$req['pick']==5||$req['pick']==6||$req['pick']==8||$req['pick']==14) {
  ...
  public function getliveoddsadmin(Request $req)
  {
    ...
    if ($totals) {
      if ($totals->pick==2||$totals->pick==3||$totals->pick==4||$totals->pick==5||$totals->pick==6||$totals->pick==8||$totals->pick==14) {
        ...
        $totalbets = expertbet::where('startingfight',$req['start'])->where('event_id',$totals->event_id)->whereIn('turn',[2,3,4,5,6,8,14])->count();
        $count2 = expertbet::where('startingfight',$req['start'])->where('event_id',$totals->event_id)->select('user_id')->whereIn('turn',[2,3,4,5,6,8,14])->groupBy('user_id')->get();
  ...
  public function closeclaimpick2(Request $req)
  {
    ...
    $result = DB::transaction(function () use($req) {
      ...
      elseif ($req['pick']==3) {
        $get1=Potmoney::where('id',$req['id'])->where('pick',3)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',3)->whereIn('winner',[1,4])->get();
      }
      elseif ($req['pick']==4) {
        $get1=Potmoney::where('id',$req['id'])->where('pick',4)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',4)->whereIn('winner',[1,4])->get();
      }
      elseif ($req['pick']==5) {
        $get1=Potmoney::where('id',$req['id'])->where('pick',5)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',5)->whereIn('winner',[1,4])->get();
      }
      elseif ($req['pick']==6) {
        $get1=Potmoney::where('id',$req['id'])->where('pick',6)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',6)->whereIn('winner',[1,4])->get();
      }
      elseif ($req['pick']==8) {
        $get1=Potmoney::where('id',$req['id'])->where('pick',8)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',8)->whereIn('winner',[1,4])->get();
      }
      elseif ($req['pick']==14) {
        $get1=Potmoney::where('id',$req['id'])->where('pick',14)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',14)->whereIn('winner',[1,4])->get();
      }
      else {
        $get1=Potmoney::where('id',$req['id'])->where('pick',3)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',3)->whereIn('winner',[1,4])->get();
      ...
      $createlogs->message = auth()->user()->username." Confirmed  CLOSE claiming bets \nstartingfight : ".$req['startingfight']."\nPick ".$req['pick'];
      $createlogs->save();
  ...
  public function close(Request $req)
  {
    ...
    $get1=Event::where('id',$req['id'])->first();
    $get=Event::where('event_name',$get1->event_name)->get();
    $array = array();
    foreach ($get as $key) {
      array_push($array, $key->id);
      ...
      $getwinners = expertbet::where('startingfight',$checker->startingfight)->whereIn('event_id',$array)->where('turn',20)->where('winner',1)->orderBy('wins','DESC')->get();
      ...
      $createlogs->user_id = auth()->user()->id;
      // $getuser1 = User::findOrFail($req['user_id']);
      $createlogs->message = auth()->user()->username." Confirmed  close claiming bets, for \nstartingfight : ".$req['startingfight']."\nPick : ".$control->pick;
      $createlogs->save();
      broadcast(new resultevent('awd',$req['startingfight'],auth()->user()->name,'confirmclaimedclose',$req['id'],auth()->user()->id,'id','id'))->toOthers();
      return $getwinners;
  }
  ...
  public function openclaimpick2(Request $req)
  {
    ...
    $result = DB::transaction(function () use($req) {
      if ($req['pick']==3) {
        $get1=Potmoney::where('id',$req['id'])->where('pick',3)->first();
        $checkbets = expertbet::where('event_id',$get1->event_id)->where('turn',3)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',3)->get();
        $getbets2 = expertbet::where('event_id',$get1->event_id)->where('turn',3)->sum('amount');
      }
      elseif ($req['pick']==4) {
        $get1=Potmoney::where('id',$req['id'])->where('pick',4)->first();
        $checkbets = expertbet::where('event_id',$get1->event_id)->where('turn',4)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',4)->get();
        $getbets2 = expertbet::where('event_id',$get1->event_id)->where('turn',4)->sum('amount');
      }
      elseif ($req['pick']==5) {
        $get1=Potmoney::where('id',$req['id'])->where('pick',5)->first();
        $checkbets = expertbet::where('event_id',$get1->event_id)->where('turn',5)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',5)->get();
        $getbets2 = expertbet::where('event_id',$get1->event_id)->where('turn',5)->sum('amount');
      }
      elseif ($req['pick']==6) {
        $get1=Potmoney::where('id',$req['id'])->where('pick',6)->first();
        $checkbets = expertbet::where('event_id',$get1->event_id)->where('turn',6)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',6)->get();
        $getbets2 = expertbet::where('event_id',$get1->event_id)->where('turn',6)->sum('amount');
      }
      elseif ($req['pick']==8) {
        $get1=Potmoney::where('id',$req['id'])->where('pick',8)->first();
        $checkbets = expertbet::where('event_id',$get1->event_id)->where('turn',8)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',8)->get();
        $getbets2 = expertbet::where('event_id',$get1->event_id)->where('turn',8)->sum('amount');
      }
      elseif ($req['pick']==14) {
        $get1=Potmoney::where('id',$req['id'])->where('pick',14)->first();
        $checkbets = expertbet::where('event_id',$get1->event_id)->where('turn',14)->first();
        $getbets = expertbet::where('event_id',$get1->event_id)->where('turn',14)->get();
        $getbets2 = expertbet::where('event_id',$get1->event_id)->where('turn',14)->sum('amount');
      }
      else {
        $get1=Potmoney::where('id',$req['id'])->where('pick',2)->first();
        $checkbets = expertbet::where('event_id',$get1->event_id)->where('turn',2)->first();
  ...
  public function open(Request $req)
  {
    ...
    if ($checker->wins===$control->pick) {
      ...
      if ($highest==20) {
        ...
        $open->payout = $get1->jackpot+$computepayoutremaining;
      ...
      if ($lowest0) {
        ...
        $open->payout = $finaltotal+$get1->jackpot;
        $open->remaining = $finaltotal;
        $open->save();
      }if ($lowest1) {
      ...
      // }else {
        ...
        $open->save();
      // }
      $finalhighestpercentage1 = $get1->jackpot/$counthighest;
