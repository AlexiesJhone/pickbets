...
class Transaction extends Controller
{
  ...
  public function getusersfrommonitoring(Request $req)
  {
    ...
    $data = User::with('group')->whereIn('role',[9,4])
...
public function eventgetgroupreport(Request $req)
  ...
  public function eventgetusersgroupreport(Request $req)
  {
    ...
    ->withSum(["expertbet AS totalrakepick2"=> function ($q) use ($arrays) {
               $q->select(DB::raw("CAST(SUM(amount*0.05) AS DECIMAL(12,2)) as totalrakepick20"))->whereIn('event_id', $arrays)->where('turn',2)->where('winner','!=',4);;
       }],"amount")
...
// >sum(DB::raw('sales.price * merchants.commission'));
  ...
  public function arenareportsmodaltotal2(Request $req)
  {
    ...
    $totalpayout2 = $this->numberPrecision($totalpayout, 2);
      ...
      // $finalrake = $key->amount * $rake;
    $a = array();
    array_push($a, array('claimedcancelled'=>$totalpayoutclaimedcancelled,'totalcancelledunclaimed'=>$totalpayoutunclaimedcancelled,'cancelledbetsamount'=>$numberofbetsamountcancelled,'cancelledbetsamountpayout'=>$numberofbetsamountcancelled,'numberofbets'=> $betcount,'totalbetsamountfinal'=> $numberofbetsamount,'rakefinal'=> $rakefinal,'totalpayout'=> $totalpayout2,'totalpayoutunclaimed'=> $totalpayoutunclaimed,'officerake'=>$totalrake,'totalpayoutpaid'=>$totalpayoutss,'breakage'=>$breakage,'pick'=>2));
    return $a;
  }
  public function arenareportsmodaltotal3(Request $req)
  {
    ...
    $totalrake2 = $pot2;
    $totalrake20 = $pot20;
    $totalcleanfunds = $numberofbetsamount20 * $funds;
    $currentfunds = $totalcleanfunds - $totaldeductedabono;
  ...
  public function arenareportsmodal2(Request $req)
  {
    ...
    $a = array();
    foreach ($pot as $key) {
      $bet = expertbet::where('event_id',$key->event_id)->where('turn',2)->where('winner','!=',0)->where('winner','!=',3)->get();
  ...
  public function totalarenareports(Request $req)
  {
    ...
    $totalamountbet2 = potmoney::where('event_id',$req['event_id'])->where('pick',20)->sum('amount');
    ...
    // $totalunclaimed = 1;
    $b = array();
    array_push($b,array('netfees2'=>$netfees2,'netfees20'=>$netfees20,'totalincome'=>$totalincome2,'totalamount'=>$totalamountbet,'totalpayout'=>$payout2,'totalunclaimed'=>$newtotalunclaimed2,'totalrake'=> $totalrake,'cleanfunds'=>$totalcleanfunds,'breakage'=>$breakage2,'current_contingency'=>$currentfunds,'totalpayouts'=>$totalpayoutss,'rakepick2'=>$totalrakepick2,'overall'=>$overallrake ));
    return $b;
  }
