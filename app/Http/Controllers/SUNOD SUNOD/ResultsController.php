...
class ResultsController extends Controller
{
  ...
  public function regrade(Request $req)
  {
    ...
    if ($getoneprebetonly||$getoneprebetonly2||$getoneprebetonly3||$getoneprebetonly4||$getoneprebetonly5||$getoneprebetonly6) {
      if ($getoneprebetonly) {
  ...
  public function confirmed(Request $req)
  {
    ...
    // $data2->save();
      ...
      if ($getoneprebetonly) {
        ...
        $nextstart = $starting;
        $checknextbets = expertbet::where('startingfight',$nextstart)->whereIn('event_id',$array)->where('turn',3)->count();
        if (!$checknextbets) {
          Potmoney::whereIn('event_id',$array)->where('startingfight',$nextstart)->where('pick',3)->update(['claim'=>1]);
          Event::where('startingfight',$nextstart)->WhereIn('id',$array)->where('pick',3)->update(['control'=>'Finished']);
        }
        $nextstart = $starting-1;
        $checknextbets = expertbet::where('startingfight',$nextstart)->whereIn('event_id',$array)->where('turn',3)->count();
        if (!$checknextbets) {
          Potmoney::whereIn('event_id',$array)->where('startingfight',$nextstart)->where('pick',3)->update(['claim'=>1]);
          Event::where('startingfight',$nextstart)->WhereIn('id',$array)->where('pick',3)->update(['control'=>'Finished']);
