...
class PotmoneyController extends Controller
{
  ...
  public function open(Request $req)
  {
    ...
    if ($checker->wins===$control->pick) {
      $open->claim = 2;
      $c1 = $control->jackpot+$control->addtojackpot;
      ...
      if ($control->addtojackpot) {
        // if add to jackpot has value
        $control->addtojackpot=$control->addtojackpot+$tojackpot2;
      }else {
        // if add to jackpot is empty
        $control->addtojackpot=$tojackpot2;
      }
      $control->save();
      $getevent->save();
      // return $maxwin2;
