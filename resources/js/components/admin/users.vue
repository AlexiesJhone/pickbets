            ...
            <div class="col-md-12">
                ...
                <div class="card-header card-header-tabs card-header-dark bg-dark">
                    ...
                    <div class="nav-tabs-wrapper">
                      <ul class="nav nav-tabs" data-tabs="tabs">
                        ...
                        <li class="nav-item" v-if="user.role===1">
                          <a class="nav-link" @click.prevent="showaddusermodal" style="cursor: pointer;">
                  ...
                  <div class="card-body table-responsive">
                    ...
                    <table class="table table-sm">
                      ...
                      <tbody>
                        <tr>
                          <td> <input type="number" oninput="this.value = Math.round(this.value);"value="" placeholder="User id" class="form-control" v-model="search.id"> </td>
                          <td>
                      ...
                      <thead>
                        <tr class="text-muted font-weight-bold">
                          <th colspan="3"><b>Role</b></th>
                          <th colspan="2"><b>Active</b></th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr>
                          <td colspan="3"><select v-model="search.role" class="form-control" id="role" required>
                            <option value="1">Admin</option>
                            <option value="9">teller</option>
                            ...
                            <option value="8">Confirm Declarator</option>
                            <option value="10">Board Admin</option>
                          </select></td>
                          <td colspan="2">
                            <select class="form-control" id="role" v-model="search.active" required>
                        ...
                        <table class="table table-sm" id="myTable">
                          ...
                          <tbody>
                            <tr v-for="t in users.data">
                              ...
                              <td>{{t.email}}</td>
                              <td>{{t.pnumber}}</td>
                              <td><a class="text-success" v-if="t.active===1">Active</a><a class="text-danger" v-if="t.active===2">Deactivated</a></td>
                              ...
                              <td> <a v-if="t.role===1">Admin</a><a v-if="t.role===9">Teller</a><a v-if="t.role===3">Mobile Player</a><a v-if="t.role===4">Cashier</a><a v-if="t.role===5">Declarator</a>
                                <a v-if="t.role===6">CSR</a><a v-if="t.role===7">Boss/Manager</a><a v-if="t.role===2">Supervisor</a><a v-if="t.role===8">Confirm Declarator</a><a v-if="t.role===10">Board Admin</a></td>
                              ...
                              <a v-if="t.role===2" class="btn btn-danger btn-sm text-white" @click.prevent='changepin(t)'>Pin</a>
                              <a class="btn btn-success btn-sm text-white"@click.prevent='showuserdetails(t)' v-if="t.role===9||t.role===4||t.role===3">View User Details</a>
                              <a v-else class="btn btn-success btn-sm text-white disabled">View User Details</a>
                             </td>
                            </tr>
                          </tbody>
                        </table>
                 ...
                 <div class="card" v-if="user.role==6">
                      ...
                      <table class="table table-hover table-stripped table-bordered">
                        ...
                        <tbody>
                          <tr v-for="t in eventsdetailedpage.data" >
                            ...
                            <td> <a class="font-weight-bold text-success" v-if="t.status==1">Active</a><a class="font-weight-bold text-info" v-if="t.status==2">Finished</a><a class="font-weight-bold text-danger" v-if="t.status==0">Pending</a> </td>
                      ...
                      <table class="table table-hover table-stripped table-bordered">
                        ...
                        <thead>
                          <tr class="">
                            ...
                            <th colspan="5" class="font-weight-bold">Fight Date</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr>
                            <td colspan="3">{{form.event_name}}</td>
                            <td colspan="5">{{form.fightdate|datec}}</td>
                          </tr>
                        </tbody>
                        <thead class="bg-dark text-warning text-center">
                          <tr>
                        ...
                        <tbody>
                          <tr v-for="s in eventsdetailedmodal.data">
                            ...
                            <td> <a v-if="s.winner===1||s.winner===2" class="text-success">Winner</a><a v-else-if="s.winner===3||s.winner===5" class="text-danger">Loss</a><a v-else class="text-danger">Pending</a> </td>
                        ...
                        <tbody>
                          <tr v-for="s in eventsdetailedmodal2.data">
                            ...
                            <td>{{s.cashier}}</td>
                            <td><a v-if="form.role===0||form.role===4">{{s.user}}<a v-if="s.barcode!=s.id">/{{s.barcode}}</a></a><a v-else-if="s.userrole===3">{{s.user}}</a> </td>
              ...
              <div class="modal fade" id="pin" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-sm" role="document">
                  <div class="modal-content" style="border:none !important;">
                    <div class="modal-header bg-dark">
                      <h5 class="modal-title text-warning font-weight-bold" id="exampleModalLabel">Pin of {{pin.username}}</h5>
                      <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true" class="text-white">&times;</span>
                      </button>
                    </div>
                    <div class="modal-body">
                      <input type="text" class="form-control" maxlength="4" v-model="pin.pin">
                      <div v-if="pin.errors.has('pin')" v-html="pin.errors.get('pin')"  class="text-danger font-weight-bold" />
                    </div>
                    <div class="modal-footer">
                      <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                      <button v-if="pin.pin && user.role===1" type="button" class="btn btn-success" @click.prevent='addpin'>Change Pin</button>
                      <button v-if="!pin.pin && user.role===1" type="button" class="btn btn-success" @click.prevent='addpin'>Add Pin</button>
                    </div>
                  </div>
                </div>
              </div>
    ...
    export default {
      ...
      methods:{
        ...
        addpin(){
          Swal.fire({
            ...
            if (result.isConfirmed) {
              this.pin.post('/pick20/addpin').then(()=>{
                this.getallusers();
                $('#pin').modal('hide');
                Swal.fire(
                  'Success!',
        ...
        checkeventdetails(t,page=1){
          ...
          this.form.fightdate = t.fightdate
          this.form.post('/pick20/checkeventdetails?page='+page).then(response=>{
            this.eventsdetailedmodal = {};
            this.eventsdetailedmodal = response.data;
            this.loading=false;
