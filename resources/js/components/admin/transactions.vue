        ...
        <div class="row justify-content-center">
          <div id="overlay" v-if="loading">
            ...
            <div class="col-md-12">
                  ...
                  <table class="table table-stripped table-hover table-bordered">
                    ...
                    <tbody>
                      <tr v-for="t in events.data">
                        ...
                        <td><a class="btn btn-sm btn-success text-white" @click.prevent='transmodal(t)'>View Transactions</a>
                          <a class="btn btn-sm btn-success text-white" @click.prevent='transmodaldownloadall(t)'>Download All Transactions</a>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                      ...
                      <table class="table table-hover table-stripped table-bordered">
                        ...
                        <thead>
                          ...
                          <tr>
                            ...
                            <th class="font-weight-bold">Groups</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr v-for="(t, index) in modal.data" :index='index+1'>
                            ...
                            <td><a v-if="t.cashiername">{{t.cashiername}}</a></td>
                            <td><a v-if="t.type==='Cash Out'||t.type==='Cash In'">-</a> <a v-else-if="t.type==='Withdrawal_Cancelled'||t.type==='Withdrawal_Mobile'||t.type==='Deposit'||t.type==='Transfered'">{{t.name}}</a> <a v-else>{{t.barcode}}</a> </td>
                            <td>{{t.groups}}</td>
                          </tr>
                        </tbody>
                        <thead>
                        ...
                        <thead>
                          <tr class="bg-dark text-warning">
                            <th colspan="3" class="font-weight-bold">Total Cash In</th>
                            <th colspan="3" class="font-weight-bold">Total Cash Out</th>
                          </tr>
                        </thead>
                        <tbody>
                          <tr v-for="(t,index) in total" :index='index'>
                            <td colspan="3" v-if="index===total.length-1">{{Number(t.totalcashin).toLocaleString('en-PH', {style:'currency', currency:'PHP'})}}</td>
                            <td colspan="3" v-if="index===total.length-1">{{Number(t.totalcashout).toLocaleString('en-PH', {style:'currency', currency:'PHP'})}}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
              ...
              <div class="modal fade" id="downloadall" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                  <div class="modal-content">
                    <div class="modal-header bg-dark">
                      <h5 class="modal-title text-warning font-weight-bold" id="exampleModalLabel">Download all {{form.event_name}} Transactions</h5>
                      <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                      </button>
                    </div>
      ...
      data(){
        return{
          ...
          modalfields: {
             ...
             'Transaction': 'type',
             'amount': 'amount',
             'Transacted By': 'cashiername',
             'Transacted To': 'reciever',
             'Group': 'groups',
           },
        }
      },
      methods:{
        transmodaldownloadall(t){
          this.form.fill(t);
          this.loading=true;
          this.form.post('/pick20/transmodaldownloadall').then(response=>{
            this.downloadall = response.data;
            if (this.downloadall.length) {
