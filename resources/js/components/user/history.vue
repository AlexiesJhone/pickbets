        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
              ...
              <session :userx='user'></session>
                <div class="card">
                    <div class="card-header bg-dark text-white">Bet History<br> <a class="text-warning">All your Pick20 bet history starting from the latest.</a> </div>
                        ...
                        <table class="table table-sm table-striped">
                          <tbody>
                            <tr v-for="(t,index) in bets.data" :index='index'>
                              <div class="card">
                                  <div class="card-header bg-dark text-warning text-center"> {{t.event_name}}<br> Fight Date : {{t.fightdate|datec}} Starting Fight : {{t.startingfight}} [Pick : {{t.pick}}]</div>
                                  <div class="card-body table-responsive" style="padding:0">
                                    <table class="table table-sm table-striped" >
                                      ...
                                      <tbody>
                                        <tr v-for="(a,indexx) in t.past_expertbet" :index='indexx' v-if="t.status==2">
                                          ...
                                          <td class="text-center"><a v-if="a.winner===0" class="text-danger">Pending</a><a v-if="a.winner===1||a.winner===2" class="text-success">+{{Number(a.result).toLocaleString(undefined, {minimumFractionDigits: 2,maximumFractionDigits: 2})}}</a>
                                            <a v-if="a.winner===3||a.winner===5" class="text-danger">-{{Number(a.amount).toLocaleString(undefined, {minimumFractionDigits: 2,maximumFractionDigits: 2})}}</a><a v-if="a.winner===4" class="text-success">Cancelled [+{{Number(a.amount).toLocaleString(undefined, {minimumFractionDigits: 2,maximumFractionDigits: 2})}}]</a></td>
                                        </tr>
                                        <tr v-for="(a,indexx) in t.expertbet" :index='indexx' v-if="t.status==0||t.status==1">
                                          <th class="text-center">{{indexx+1}}</th>
                                           <td class=" text-center">{{Number(a.amount).toLocaleString()}}</td>
                                          <td class="text-center">{{a.bet}}</td>
                                          <td class="text-center">{{a.wins}}</td>
