                  ...
                  <div class="modal-content modal-content1" style="border:none">
                    ...
                    <div class="modal-body modal-body1 table-responsive" style="padding:0">
                      <table class="table table-hover table-stripped table-bordered">
                        ...
                        <tbody>
                          <tr v-for="(t, index) in modal" :index='index+1'>
                            <td>{{t.created_at|datef}}</td>
                            <td>{{t.type}}</td>
                            <td class="text-success">{{Number(t.amount).toLocaleString('en-PH', {style:'currency', currency:'PHP'})}}</td>
                            <td>{{t.cashier.username}}</td>
                            <td><a v-if="t.type==='Cash Out'||t.type==='Cash In'">-</a> <a v-else-if="t.type==='Withdrawal'||t.type==='Deposit'">{{t.user.name}}</a> <a v-else>{{t.barcode}}</a> </td>
    ...
    export default {
      ...
      methods:{
        transmodal(t){
          this.form.fill(t);
          this.loading=true;
          this.form.post('/pick20/transmodal').then(response=>{
        ...
        geteventswithtransactions(){
          this.loading=true;
          axios.get('/pick20/geteventswithtransactions').then(response=>{
            this.loading=false;
